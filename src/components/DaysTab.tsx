...
import type { TripState, VoteDir } from '../lib/store'
...
export const DaysTab = ({
  ...
  const suggestions = state.suggestions
    .filter((suggestion) => suggestion.dayId === day.id)
    .sort((a, b) => b.votes - a.votes)

  const submit = () => {
    if (!draft.trim()) return
    onAdd(day.id, draft.trim(), draftTime)
    setDraft('')
    setDraftTime('')
  }
    ...
    <>
      <div className="day-strip">
        {days.map((entry) => (
          <button
            key={entry.id}
            className={entry.id === day.id ? 'day-pill active' : 'day-pill'}
            onClick={() => setDay(entry)}
            aria-current={entry.id === day.id}
          >
                ...
                <div className="item-head">
                  ...
                  >
                    ...
                    {!expanded && notes.length > 0 && (
                      <span className="note-count" aria-label={plural(notes.length, 'note')}>
                        {notes.length}
                      </span>
                    )}
          ...
          <div className="dashed">
            <div className="dashed-head">
              ...
              <span className="count">
                {suggestions.length === 0 ? 'none yet' : plural(suggestions.length, 'idea')}
              </span>
            </div>
